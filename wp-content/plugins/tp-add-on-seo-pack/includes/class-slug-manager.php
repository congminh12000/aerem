...
class TRP_SP_Slug_Manager {
        ...
        if( class_exists( 'WooCommerce' ) ) {
            ...
            if( class_exists('TRP_Translation_Manager') ) {
                if ($name === 'product' || $name === 'product_cat' || $name === 'product_tag') {
                    if ($name === 'product' && trim($args['rewrite']['slug'], '/\\') === TRP_Translation_Manager::strip_gettext_tags(_x('product', 'slug', 'woocommerce')))
                        $name = trim($name, '/\\');
                    elseif ($name === 'product_cat' && trim($args['rewrite']['slug'], '/\\') === TRP_Translation_Manager::strip_gettext_tags(_x('product-category', 'slug', 'woocommerce')))
                        $name = 'product-category';
                    elseif ($name === 'product_tag' && trim($args['rewrite']['slug'], '/\\') === TRP_Translation_Manager::strip_gettext_tags(_x('product-tag', 'slug', 'woocommerce')))
                        $name = 'product-tag';
    ...
    public function get_translated_rewrite_base_slug( $name, $langauge, $is_tax ){
        ...
        if( $is_tax ){
            ...
            //eliminate all translation pairs from db that are no longer registered (the taxonomies don't exist) so they don't cause problems
            ...
            if( !empty( $current_taxonomies_slugs ) && is_array($current_taxonomies_slugs) ){
                ...
                foreach ($current_taxonomies_slugs as $key => $value) {
                    $current_taxonomies_slugs[$key] = trim($value, '/\\');
                }
                ...
                if( !empty( $trp_taxonomy_slug_translation ) && is_array($trp_taxonomy_slug_translation) ) {
                    foreach ($trp_taxonomy_slug_translation as $tax_slug => $tax_translations) {
                        if (!in_array(trim($tax_slug, '/\\'), $current_taxonomies_slugs)) {
            ...
            if( !empty( $current_cpt_slugs ) && is_array($current_cpt_slugs) ){
                //remove any slashes from slugs as we need to compare them without slashes
                foreach ($current_cpt_slugs as $key => $value) {
                    $current_cpt_slugs[$key] = trim($value, '/\\');
                }
                ...
                if( !empty( $trp_cpt_slug_translation ) && is_array($trp_cpt_slug_translation) ) {
                    foreach ($trp_cpt_slug_translation as $cpt_slug => $cpt_translations) {
                        if (!in_array(trim($cpt_slug, '/\\'), $current_cpt_slugs)) {
        ...
        if (!empty($slug_translations)) {
            ...
            if( !empty( $slug_translations[$name] ) && !empty( $slug_translations[$name]['translationsArray'] ) && !empty( $slug_translations[$name]['translationsArray'][$langauge] ) && !empty( $slug_translations[$name]['translationsArray'][$langauge]['translated'] ) ){
                return sanitize_title( trim( $slug_translations[$name]['translationsArray'][$langauge]['translated'], '/\\' ) );
            }
        }
    ...
    public function get_taxonomy_translated_slugs_pairs_for_languages( $from_language, $to_langauge ){
        global $trp_taxonomy_slug_translation, $TRP_LANGUAGE, $trp_taxonomy_slug_translation_transient;
