<?php
	...
	class WS_Form_Common {
		...
		public static function wrapper_classes() {
			...
			// Output classes
			WS_Form_Common::echo_esc_attr(implode(' ', $wrapper_classes_array));
		}
		...
		public static function attributes_e($attributes) {
			...
			foreach($attributes as $key => $value) {
				...
				WS_Form_Common::echo_esc_attr($key);
				echo '="';	// phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped
				WS_Form_Common::echo_esc_attr($value);
				echo '"';	// phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped
			}
		}
		...
		public static function error_nonce() {
			...
			// Send response
			WS_Form_Common::echo_wp_json_encode($response);
		...
		// Get IP lookup
		public static function get_ip_lookup($var_array, $ip_lookup_method = '') {
				...
				if($ip_lookup !== false) {
					...
					switch($ip_lookup_method) {
						...
						case 'ipinfo' :
							...
							// Check for API key
							$ip_lookup_ipinfo_key = sanitize_text_field(WS_Form_Common::option_get('ip_lookup_ipinfo_key'));

							if(!empty($ip_lookup_ipinfo_key)) {

								// Commercial use
								$url = sprintf('https://ipinfo.io/%s/json?token=%s', $ip_lookup, $ip_lookup_ipinfo_key);

							} else {

								// Non-commercial use
								$url = sprintf('https://ipinfo.io/%s/json', $ip_lookup);
							}
						...
						case 'ipapi' :
							...
							// Check for API key
							$ip_lookup_ipapi_key = sanitize_text_field(WS_Form_Common::option_get('ip_lookup_ipapi_key'));

							if(!empty($ip_lookup_ipapi_key)) {

								// Commercial use
								$url = sprintf('https://pro.ip-api.com/json/%s?fields=country,region,city,zip,lat,lon,timezone&key=%s', $ip_lookup, $ip_lookup_ipapi_key);

							} else {

								// Non-commercial use (HTTP only)
								$url = sprintf('http://ip-api.com/json/%s?fields=country,region,city,zip,lat,lon,timezone', $ip_lookup);
							}
							break;
						...
						case 'ipapico' :
							...
							// Check for API key
							$ip_lookup_ipapico_key = sanitize_text_field(WS_Form_Common::option_get('ip_lookup_ipapico_key'));

							if(!empty($ip_lookup_ipapico_key)) {

								// Commercial use
								$url = sprintf('https://ipapi.co/json/%s?key=%s', $ip_lookup, $ip_lookup_ipapico_key);

							} else {

								// Non-commercial use (HTTP only)
								$url = sprintf('http://ipapi.co/json/%s', $ip_lookup);
							}
							break;
						...
						default :
							...
							// Check for API key
							$ip_lookup_geoplugin_key = sanitize_text_field(WS_Form_Common::option_get('ip_lookup_geoplugin_key'));

							if(!empty($ip_lookup_geoplugin_key)) {

								$url = sprintf('https://ssl.geoplugin.net/json.gp?ip=%s&k=%s', $ip_lookup, $ip_lookup_geoplugin_key);

							} else {

								$url = sprintf('http://www.geoplugin.net/json.gp?ip=%s', $ip_lookup);
							}
					}
					...
					$args = array(
						...
						'headers' => array(
							...
							// Set referrer. PHP CURL defaults to request URL.
							'referer' => sanitize_url(WS_Form_Common::get_http_env_raw(array('HTTP_REFERER')))
						),
					...
					if(!is_wp_error($wp_remote_get_response)) {
						...
						// Build data
						self::$ip_lookup_response = json_decode($ip_lookup_response);

						if(is_null(self::$ip_lookup_response)) {

							// Set to true if error so we don't request again
							self::$ip_lookup_response = true;

						} else {

							// Convert to array
							self::$ip_lookup_response = (array) self::$ip_lookup_response;
						}

					} else {

						// Set to true so we don't make any further requests
						self::$ip_lookup_response = true;
					}
				}
			}

			// Lookup geo data and return the result
			if(is_array(self::$ip_lookup_response)) {

				$return_array = array();

				foreach($var_array as $var) {

					if(isset(self::$ip_lookup_response[$var])) {

						$return_array[] = self::$ip_lookup_response[$var];
					}
				}
		...
		public static function wsf_add_query_args($params, $url) {
			...
			foreach($params as $key => $value) {
				...
				// We don't want this because some API's (e.g. AWeber) require periods in keys
		...
		// Get current URL (Used by add-ons during oAuth)
		public static function get_current_url() {

			// Check $_SERVER global exists
			...
			// Add hostname
			if(isset($_SERVER['HTTP_HOST'])) { $request_url .= $_SERVER['HTTP_HOST']; }

			// Add request URI
			if(isset($_SERVER['REQUEST_URI'])) { $request_url .= sanitize_url($_SERVER['REQUEST_URI']); }
		...
		public static function parse_variables_ecommerce($submit) {
			...
			uasort($ecommerce_rows, function ($ecommerce_rows_1, $ecommerce_rows_2) {
				...
				return ($ecommerce_rows_1['priority'] == $ecommerce_rows_2['priority']) ? 0 : (($ecommerce_rows_1['priority'] < $ecommerce_rows_2['priority']) ? -1 : 1);
			});
...
<script>
	...
	function wsf_review_nag_dismiss() {
		...
		(function($) {
			...
			$.ajax({ method: 'POST', url: '<?php WS_Form_Common::echo_esc_html(self::get_api_path('helper/review-nag/dismiss/')); ?>', data: { '_wpnonce': '<?php WS_Form_Common::echo_esc_attr(wp_create_nonce('wp_rest')); ?>' } });
...
<?php
			...
			if($format_output === false) {
				...
				switch($input_type_datetime) {
					...
					case 'date' :
						...
						return gmdate($format_date, strtotime($date));

					case 'month' :

						return gmdate('F Y', strtotime($date));

					case 'time' :

						return gmdate($format_time, strtotime($date));

					case 'week' :

						return __('Week', 'ws-form') . ' ' . gmdate('W, Y', strtotime($date));

					default :

						return gmdate($format_date . ' ' . $format_time, strtotime($date));
				}

			} else {

				return gmdate($format_output, strtotime($date));
			}
		}
				...
				foreach($group['variables'] as $item_id => $item) {
					...
					switch($type) {
						...
						case 'date' :
							...
							$value = ($value != '') ? gmdate(get_option('date_format'), $value) : '-';
							break;
					}
		...
		public static function get_currency() {
			...
			$currency = self::option_get('currency', WS_Form_Common::get_currency_default());
		...
		public static function csv_file_to_data_grid_meta_value($file, $meta_key, $meta_value) {
			...
			while($row = fgetcsv($fp)) {
				...
				// Check for duplicate row ID's (Attempt to fix import data errors)
				if($row_id > 0) {
		...
		// Set cookie
		public static function cookie_set($cookie_name, $cookie_value = '', $cookie_expiry = true, $form_id = false) {

			// Cookie name
			$cookie_prefix = WS_Form_Common::option_get('cookie_prefix', '');
			if($cookie_prefix == '') { return false; }

			$cookie_name = sprintf('%s%s_%s', $cookie_prefix, (($form_id !== false) ? sprintf('_%u', $form_id) : ''), $cookie_name);
			...
			for($index = 0; $index < $count; $index++) {
				...
				if(is_array($lookup_value)) {
					...
					foreach($lookup_value as $lookup_value_single) {
						...
						$date_string = str_replace(strtolower($lookup_value_single), $replacements[$index], $date_string);
					}

				} else {

					// Straight string swap
					$date_string = str_replace(strtolower($lookup_value), $replacements[$index], $date_string);
				}
