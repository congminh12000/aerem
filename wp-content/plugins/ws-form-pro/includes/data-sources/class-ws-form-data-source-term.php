				...
				foreach($terms as $term_index => $term) {
					...
					foreach($columns as $column) {
						...
						switch($column) {
							...
							case 'id' : $column_value = strval($term_id); break;
...
//				['data_source_recurrence']
		...
		public function config_meta_keys($meta_keys = array(), $form_id = 0) {
			...
			foreach($taxonomies as $taxonomy) {
				...
				if(empty($taxonomy_label)) { $taxonomy_label = $taxonomy_name; }

				$config_meta_keys['data_source_' . $this->id . '_taxonomies']['options'][] = array(

					'value' => $taxonomy->name,
					'text' => sprintf('%s (%s)', $taxonomy_label, $taxonomy_name)
				);
			}
