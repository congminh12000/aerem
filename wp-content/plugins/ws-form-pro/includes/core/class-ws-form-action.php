<?php
	...
	abstract class WS_Form_Action {
			...
			$config_meta_keys_action = array(
				...
				'form_populate_tag_mapping'	=> array(
					...
					'help'						=>	sprintf(
						...
						/* translators: %s = WS Form */
						__('Map list tags to %s fields.', 'ws-form'),
		...
		public function api_call($endpoint, $path = '', $method = 'GET', $body = null, $headers = array(), $authentication = 'basic', $username = false, $password = false, $accept = 'application/json', $content_type = 'application/json', $timeout = WS_FORM_API_CALL_TIMEOUT, $ssl_verify = WS_FORM_API_CALL_SSL_VERIFY, $cookies = array()) {
		...
		public static function update_form($form_id, $action_id, $list_id, $list_sub_id = false, $list = false, $list_fields = false, $list_fields_meta_data = false, $form_fields = false, $form_actions = false, $form_conditionals = false, $form_meta = false) {
			...
			// Get add form conditionals
			$meta_conditional = $meta_keys['conditional']['default'];

			if(
				($form_conditionals === false) &&
				method_exists($action, 'get_conditionals')
			) {

				$form_conditionals = $action->get_conditionals($form_field_id_lookup_all, $form_field_type_lookup);
			}

			if(is_array($form_conditionals)) {

				$form_conditional_index = 1;

				foreach($form_conditionals as $form_conditional) {

					// Add conditional
					$meta_conditional['groups'][0]['rows'][] = self::update_form_conditional($form_conditional_index++, $form_conditional, $form_field_id_lookup);
				}
			}
			// Form meta
			$meta = array(
