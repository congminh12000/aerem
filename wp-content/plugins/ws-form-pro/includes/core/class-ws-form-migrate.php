<?php
	...
	class WS_Form_Migrate extends WS_Form_Core {
		...
		public function is_data($id) {
			...
			if(count($config['detect_table']) === 0) { return true; }
			...
			foreach($config['detect_table'] as $table) {
				...
				if(!$wpdb->get_var($sql)) { $tables_found = false; break; }
			}
		...
		public function get_data($part, $form_id = false, $offset = false, $meta_map = false, $count = false) {
			...
			if($count) {
				...
				$sql = sprintf(
					...
					"SELECT COUNT(%s) as records_total FROM %s%s%s;",
					$sql_record_count,
					$sql_record_from,
					$sql_record_join,
					$sql_record_where
				...
				$records_total = $wpdb->get_var($sql);	// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
				...
				$sql = sprintf(
					...
					"SELECT SQL_CALC_FOUND_ROWS %s FROM %s%s%s%s%s%s;",
					$sql_record_select,
					$sql_record_from,
					$sql_record_join,
					$sql_record_where,
				...
				$table_records = $wpdb->get_results($sql, 'ARRAY_A');	// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
			}

			// Counters
			$records_total = absint($wpdb->get_var('SELECT FOUND_ROWS()'));	// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
			...
			if(isset($table_record['sql_transpose'])) {
				...
				switch($data_format) {
					...
					case 'serialize' :
						...
						if(!is_serialized($table_records)) { return false; }
						$table_records = unserialize($table_records);
						break;
				...
				if(is_object($table_records)) { $table_records = json_decode(wp_json_encode($table_records), true); }
				...
				if($table_metadata !== false) {
					...
					if(isset($table_metadata['sql_transpose'])) {
						...
						switch($data_format) {
							...
							case 'serialize' :
								...
								if(!is_serialized($meta_records)) { return false; }
								$meta_records = unserialize($meta_records);
								break;
						...
						if(is_object($meta_records)) { $meta_records = json_decode(wp_json_encode($meta_records), true); }
					...
					if(count($this->id_to_sub_meta_keys) > 0) {
						...
						foreach($meta_records as $key => $record) {
							...
							if(is_object($record)) { $record = json_decode(wp_json_encode($record), true); }
				...
				if($create_group && ($part !== 'form')) {
					...
					if($part == 'group') {
						...
						$data[$return_group_index] = array(
							...
							'label'			=>	($return_lookups['label'] != '') ? $return_lookups['label'] : __('Tab', 'ws-form'),
							'meta'			=>	array(
				...
				if($create_section && ($part !== 'form')) {
					...
					if($part == 'section') {
						...
						$data[$return_section_index] = array(
							...
							'label'			=>	($return_lookups['label'] != '') ? $return_lookups['label'] : __('Section', 'ws-form'),
		...
		public function map_process(&$return_single, &$return_lookups, &$return_actions, &$return_group_index, &$create_group, &$records_since_create_group, &$return_section_index, &$create_section, $map, $meta_value, $table_records_base, &$table_records, &$table_record_key, $part_config, $source, $plugin_variables, $meta_key_key = false, $meta_value_key = false) {
				...
				foreach($map['process'] as $processes) {
					...
					switch($process) {
						...
						case 'datetime_to_html5' :
							...
							$meta_value = gmdate('Y-m-d',$meta_value_time) . "T" . gmdate('H:i:s',$meta_value_time);
						...
						case 'upload_url_to_file' :
							...
							foreach($meta_value_array as $meta_value_array_key => $meta_value) {
								...
								$meta_value = trim($meta_value);
								$url_parsed = wp_parse_url($meta_value);
			...
			switch($map['type']) {
				...
				case 'records' :
					...
					$column_count = absint(WS_Form_Common::option_get('framework_column_count', 12));
					...
					if(isset($map['merge'])) {
						...
						foreach($map['merge'] as $key => $merge_record) {
							...
							if(isset($merge_record['size_percent'])) {
								...
								$record['breakpoint_size_25'] = round($column_count * (absint($merge_record['size_percent']) / 100));
							}
							...
							foreach($map['partial_merge'] as $partial) {
								...
								if(self::ends_with($record[$partial['id']], $partial['id_partial'])) {
									...
									if(isset($partial['size_percent'])) {
										...
										$record['breakpoint_size_25'] = round($column_count * (absint($partial['size_percent']) / 100));
									}
				...
				case 'meta-submit' :
					...
					if(isset($map['destination'])) {
						...
						$key = $map['destination'];
						$return_single['meta'][WS_FORM_FIELD_PREFIX . $key] = array(
...
//			include WS_FORM_PLUGIN_DIR_PATH . 'includes/core/migrate/formidable.php';
			...
			do {
				...
				foreach($data_lookup as $data_current_key => $data_lookup) {
					...
					if(!is_array($data_lookup)) {
						...
						$data_index = absint(WS_Form_Common::mask_parse($data_lookup, $return_lookups));
