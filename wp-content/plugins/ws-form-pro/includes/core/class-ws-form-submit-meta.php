<?php
	...
	class WS_Form_Submit_Meta extends WS_Form_Core {
		...
		public function db_dupe_check($form_id, $field_id, $value, $period = false) {
			...
			$sql_prepare = sprintf('SELECT %1$s.id FROM %2$s LEFT JOIN %1$s ON %2$s.id = %1$s.parent_id WHERE %2$s.form_id = %%d AND %1$s.field_id = %%d AND %1$s.meta_value = %%s%3$s LIMIT 1;', self::get_table_name(), $table_name_submit, $period_sql);
