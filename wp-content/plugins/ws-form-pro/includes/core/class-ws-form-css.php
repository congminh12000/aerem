<?php
	...
	class WS_Form_CSS {
		...
		public function skin_color_shade_set($name, $color) {
			...
			$this->{sprintf('color_%s_light_40', $name)} = WS_Form_Common::hex_lighten_percentage($color, 40);
			$this->{sprintf('color_%s_light_85', $name)} = WS_Form_Common::hex_lighten_percentage($color, 85);
			$this->{sprintf('color_%s_dark_10', $name)} = WS_Form_Common::hex_darken_percentage($color, 10);
			$this->{sprintf('color_%s_dark_20', $name)} = WS_Form_Common::hex_darken_percentage($color, 20);
			$this->{sprintf('color_%s_dark_40', $name)} = WS_Form_Common::hex_darken_percentage($color, 40);
			$this->{sprintf('color_%s_dark_60', $name)} = WS_Form_Common::hex_darken_percentage($color, 60);
		}
		...
		public function get_conversational($css_minify = null, $force_build = false, $rtl = false) {
			...
			if($css_compile && !$force_build) {
				...
				if($css_minify) {
					...
					$css_return = WS_Form_Common::option_get(sprintf('css_public_conversational%s_min', $rtl_suffix));

				} else {

					$css_return = WS_Form_Common::option_get(sprintf('css_public_conversational%s', $rtl_suffix));
				}

			} else {

				include_once 'css/class-ws-form-css-conversational.php';

				$ws_form_css_conversational = new WS_Form_CSS_Conversational();
				...
				if($rtl) {
					...
					// Render RTL skin
					$ws_form_css_conversational->render_conversational_rtl();
				}
				...
				// Apply filters
				$css_return = apply_filters('wsf_css_get_conversational', $css_return);
		...
		// Loader
		public function get_loader($css_minify = null, $force_build = false, $rtl = false) {
			...
			if($css_compile && !$force_build) {
				...
				if($css_minify) {
					...
					$css_return = WS_Form_Common::option_get(sprintf('css_public_loader%s_min', $rtl_suffix));

				} else {

					$css_return = WS_Form_Common::option_get(sprintf('css_public_loader%s', $rtl_suffix));
				}

			} else {

				include_once 'css/class-ws-form-css-loader.php';

				$ws_form_css_loader = new WS_Form_CSS_Loader();
				...
				if($rtl) {
					...
					// Render RTL skin
					$ws_form_css_loader->render_loader_rtl();
				}
				...
				// Apply filters
				$css_return = apply_filters('wsf_css_get_loader', $css_return);
		...
		public function build_public_css() {
			...
			if($css_compile) {
				...
				// Build loader CSS
				self::build_public_css_set('get_loader', 'loader', $dir);
			}
