<?php
	...
	class WS_Form_Encryption extends WS_Form_Core {
		...
		public function encrypt($input, $key = false) {
			...
			if(!$this->can_encrypt && ($key === false)) { return $input; }

			if($key === false) { $key = $this->key; }

			return \Defuse\Crypto\Crypto::encrypt($input, \Defuse\Crypto\Key::loadFromAsciiSafeString($key));
		}

		public function decrypt($input, $key = false) {

			if(!$this->can_decrypt && ($key === false)) { return $input; }
