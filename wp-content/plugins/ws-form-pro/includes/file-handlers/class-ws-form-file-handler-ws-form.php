<?php
	...
	class WS_Form_File_Handler_WS_Form extends WS_Form_File_Handler {
		...
		public function get_url($file_object, $field_id = 0, $file_object_index = 0, $submit_hash = '', $section_repeatable_index = 0) {
			...
			return WS_Form_Common::get_api_path('helper/file_download', sprintf('hash=%s&field_id=%u&file_index=%u&section_repeatable_index=%u&_wpnonce=%s&%s=%s', rawurlencode($submit_hash), rawurlencode($field_id), rawurlencode($file_object_index), rawurlencode($section_repeatable_index), wp_create_nonce('wp_rest'), rawurlencode(WS_FORM_POST_NONCE_FIELD_NAME), rawurlencode(wp_create_nonce(WS_FORM_POST_NONCE_ACTION_NAME))));
		}
					...
					switch($file_type) {
						...
						case 'image/svg+xml':
							...
							if($file_links) {
								...
								$value_array[] = sprintf('<a href="%s" target="_blank">%s</a>', $file_url, $file_data);
						...
						default :
							...
							if($file_links) {
								...
								$value_array[] = sprintf('<a href="%s" target="_blank"><img src="data:%s;base64,%s" style="max-width: 100%%;"%s /></a>', $file_url, $file_type, base64_encode($file_data), $data_type_attribute);

							} else {

								$value_array[] = sprintf('<img src="data:%s;base64,%s" style="max-width: 100%%;"%s />', $file_type, base64_encode($file_data), $data_type_attribute);
							}
					}
				}
			}
