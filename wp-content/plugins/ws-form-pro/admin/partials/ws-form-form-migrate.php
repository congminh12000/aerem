<?php
	...
	$wsf_form_migrate = WS_Form_Common::get_query_var('wsf_form_migrate', array());
...
<h1><?php esc_html_e('Migrate', 'ws-form') ?></h1>
...
<?php
	...
	// Check migrate ID exists
	if(!isset($migrations[$wsf_form_migrate['id']])) {
...
?><option value="<?php WS_Form_Common::echo_esc_attr($migration['id']); ?>"<?php

		if(($wsf_form_migrate['id'] === false) && $migration['select']) { $wsf_form_migrate['id'] = $migration['id']; }
		if($wsf_form_migrate['id'] == $migration['id']) { ?> selected<?php }
		if(!$migration['data']) { ?> disabled="disabled"<?php }

?>><?php WS_Form_Common::echo_esc_html($migration['label']); ?> (<?php WS_Form_Common::echo_esc_html($migration['version']); ?>)<?php
...
?><option value="<?php WS_Form_Common::echo_esc_attr($form['id']); ?>"<?php if($wsf_form_migrate['form_id_source'] == $form['id']) { ?> selected<?php } ?>><?php WS_Form_Common::echo_esc_html($form['label']); ?> (<?php esc_html_e('ID', 'ws-form'); ?>: <?php WS_Form_Common::echo_esc_html($form['id']); ?>)</option>
<?php
...
<option value="form"<?php if($wsf_form_migrate['method'] == 'form') { ?> selected<?php } ?>>Form</option>
<option value="submissions"<?php if($wsf_form_migrate['method'] == 'submissions') { ?> selected<?php } ?><?php if($record_count == 0) { ?> disabled<?php } ?>><?php esc_html_e('Submissions', 'ws-form'); ?> (<?php WS_Form_Common::echo_esc_html(sprintf(_n('%u record', '%u records', $record_count, 'ws-form'), $record_count)); ?>)</option>
</select></td>
</tr>
<?php
	}
...
?>
<tr>
<th scope="row"><label for="wsf-migrate-form-id-destination"><?php
...
?><option value="<?php WS_Form_Common::echo_esc_attr($form['id']); ?>"<?php if($form['id'] == $wsf_form_migrate['form_id_destination']) { ?> selected<?php } ?>><?php WS_Form_Common::echo_esc_html($form['label']) . ' (' . esc_html__('ID', 'ws-form') . ': ' . WS_Form_Common::echo_esc_html($form['id']) . ')'; ?></option>
<?php
			}
?>
</select></td>
...
<option value=""><?php esc_html_e('Select...', 'ws-form'); ?></option>
<option value="update"<?php if($wsf_form_migrate['duplicate'] == 'update') { ?> selected<?php } ?>>Update duplicate</option>
<option value="create"<?php if($wsf_form_migrate['duplicate'] == 'create') { ?> selected<?php } ?>>Add as new duplicate</option>
<option value="ignore"<?php if($wsf_form_migrate['duplicate'] == 'ignore') { ?> selected<?php } ?>>Ignore and do not import</option>
</select></td>
</tr>
<?php
	}
...
<div data-action="wsf-field-mapping-add"><div <?php WS_Form_Common::tooltip_e(__('Add Row', 'ws-form'), 'left'); ?>><?php WS_Form_Common::render_icon_16_svg('plus-circle'); ?></div></div>
...
<script>
	...
	(function($) {
		...
		$(function() {
			...
			// Select changes
			$('[data-action="wsf-submit"]').on('change', function(e) {

				$('#wsf_changed').val($(this).attr('data-id'));

				$('#wsf-migrate-button').prop('disabled', true);

				wsf_obj.loader_on();

				$('#wsf-migrate-form').trigger('submit');
			});

			// Delete field mapping row
			$('[data-action="wsf-field-mapping-delete"] > svg').on('click', function(e) {
			...
			$('[data-action="wsf-field-mapping-add"] > div').on('click', function(e) {
				...
				$('#wsf_field_mapping tbody').append(blank_row).each(function() {
					...
					$('[data-action="wsf-field-mapping-delete"]', $(this)).on('click', function(e) {
					...
					$('[name^="wsf_field_mapping"]', $(this)).on('change', function() {
			...
			// Start migrate
			$('[data-action="wsf_migrate"]').on('click', function(e) {
			...
			$('[name^="wsf_field_mapping"]').on('change', function() {
			...
			function wsf_import_button() {
				...
				var button_enable = (<?php WS_Form_Common::echo_esc_html($show_field_mapping ? 'true' : 'false'); ?> && field_mapping_valid) || <?php WS_Form_Common::echo_esc_html(($wsf_form_migrate['method'] == 'form') ? 'true' : 'false'); ?>;
			...
			function wsf_field_mapping_save() {
				...
				wsf_obj.api_call('migrate/field-mapping', 'POST', form_data, function(response) {
...
<?php
	foreach($settings as $setting) {
?>
					'wsf_form_migrate[<?php WS_Form_Common::echo_esc_html($setting); ?>]' : $('[name="wsf_form_migrate[<?php WS_Form_Common::echo_esc_html($setting); ?>]"]').val(),
<?php
	}
?>
					'wsf_field_mapping[source][]' : $('[name="wsf_field_mapping[source][]"]').map(function() {return $(this).val();}).get()
...
?>
						...
						$('#wsf-migrate-button').prop('disabled', false);
...
?>
				...
				wsf_obj.api_call('migrate/submission/import', 'POST', form_data, function(response) {
					...
					if(remaining) {
						...
						success_message_array.push('<a href="admin.php?page=ws-form-submit&id=<?php WS_Form_Common::echo_esc_attr($wsf_form_migrate['form_id_destination']); ?>" target="_blank">' + $.WS_Form.this.language('migrate_view_submissions') + '</a>');
						var success_message = success_message_array.join(' ');
...
?><option value="<?php WS_Form_Common::echo_esc_attr($field_id); ?>"<?php if(($field_id_source !== false) && ($field_id_source == $field_id)) { ?> selected<?php } ?>><?php WS_Form_Common::echo_esc_html($field_data['label']); ?> (ID: <?php WS_Form_Common::echo_esc_html($field_id); ?>)</option>
<?php
		}
?>
</select>
...
?><option value="<?php WS_Form_Common::echo_esc_attr($field->id); ?>"<?php if(($field_id_destination !== false) && ($field_id_destination == $field->id)) { ?> selected<?php } ?>><?php WS_Form_Common::echo_esc_html($field->label); ?> (<?php WS_Form_Common::echo_esc_html($field->id); ?>)</option>
<?php
		}
?>
</select>
...
<td data-icon><div data-action="wsf-field-mapping-delete" <?php WS_Form_Common::tooltip_e(__('Delete', 'ws-form'), 'top-right'); ?>><?php WS_Form_Common::render_icon_16_svg('delete'); ?></div></td>
</tr>
<?php
	}
