<?php
	...
	class WS_Form_API_Form_Stat extends WS_Form_API {
		...
		public function api_get_chart_data($parameters) {
			...
			// Get form ID
			$form_id = absint(WS_Form_Common::get_query_var_nonce('form_id', '', $parameters));
			...
			$time_from_utc = ($ws_form_form_stat->date_ranges[$date_range]['time_from'] !== false) ? $ws_form_form_stat->get_utc_time_from($ws_form_form_stat->date_ranges[$date_range]['time_from']) : false;
			$time_to_utc = ($ws_form_form_stat->date_ranges[$date_range]['time_to'] !== false) ? $ws_form_form_stat->get_utc_time_to($ws_form_form_stat->date_ranges[$date_range]['time_to']) : false;
			...
			switch($data_type) {
				...
				case 'time' :
					...
					$chart_data = $ws_form_form_stat->db_get_chart_data_time($time_from_utc, $time_to_utc);
					break;

				case 'total' :

					$chart_data = $ws_form_form_stat->db_get_chart_data_total($time_from_utc, $time_to_utc);
					break;
			}
			...
			if($chart_data === false) {
				...
				WS_Form_Common::echo_wp_json_encode(array('error' => true));
				...
				WS_Form_Common::echo_wp_json_encode(array('error' => false, 'config' => $config));
			}
		...
		public function api_add_view($parameters) {
			...
			// Get form ID
			$form_id = absint(WS_Form_Common::get_query_var_nonce('wsffid', '', $parameters));
		...
		// API - Reset statistics for a form
		public function api_reset($parameters) {
			...
			$ws_form_form = new WS_Form_Form();
			$ws_form_form->id = $form_id;
			$ws_form_form->db_count_reset();

			return array('error' => false);
		}

		// API - Send form statistics report
		public function api_report_form_statistics_send($parameters) {
