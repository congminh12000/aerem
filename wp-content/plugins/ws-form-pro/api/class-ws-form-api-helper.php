<?php
	...
	class WS_Form_API_Helper extends WS_Form_API {
		...
		public function api_file_download($parameters) {
			...
			if(!WS_Form_Common::check_submit_hash($hash)) { die(__('Hash not specified', 'ws-form')); }

			// Get field ID
			$field_id = absint(WS_Form_Common::get_query_var_nonce('field_id', '', $parameters));
			if($field_id == 0) { die(__('Field ID not specified', 'ws-form')); }
			...
			if($file_index < 0) { die(__('File index invalid', 'ws-form')); }
			...
			// Get files
			if(!isset($field['value'])) { die(__('Field data not found', 'ws-form')); }
			$files = $field['value'];

			// Get file
			if(!isset($files[$file_index])) { die(__('Field data not found', 'ws-form')); }
			$file = $files[$file_index];

			// Get file name
			if(!isset($file['name'])) { die(__('File name not found', 'ws-form')); }
			$file_name = $file['name'];

			// Get file type
			if(!isset($file['type'])) { die(__('File type not found', 'ws-form')); }
			$file_type = $file['type'];

			// Get file path
			if(!isset($file['path'])) { die(__('File path not found', 'ws-form')); }
			$file_path = $file['path'];
		...
		public function api_intro($paramters) {
			...
			$hints = [
				...
				[
					'hint' 			=> sprintf('<strong>%s</strong><br />%s', __('Conditional Logic', 'ws-form'), __('Use conditional logic to make your form interactive! For example, you could show or hide sections of a form to make it easier to complete.', 'ws-form')),
					'element' 		=> '[data-action-sidebar="conditional"]',
					'sidebar_open' 	=> 'conditional',
				...
				[
					'hint' 			=> sprintf('<strong>%s</strong><br />%s', __('Support', 'ws-form'), __('Need help? Click here to browse and search the knowledge base or send us a support ticket.', 'ws-form')),
				...
				[
					'hint' 			=> sprintf('<strong>%s</strong><br />%s', __('Form Settings', 'ws-form'), __('Form settings include tracking, spam settings, submission count limiting and scheduling. You can also add custom CSS classes and edit the behavior of the form.', 'ws-form')),
		...
		// API - Debug close
		public function api_debug_close($parameters) {

			WS_Form_Common::option_set('helper_debug', 'off');
