<?php
	...
	class WS_Form_API_Migrate extends WS_Form_API {
		...
		public function api_form_import($parameters) {
			...
			// Create form
			$form_object = json_decode(wp_json_encode($form));
			...
			WS_Form_Common::echo_wp_json_encode($return_array);
			exit;
		}
		...
		public function api_submission_import($parameters) {
			...
			// Return variables
			$offset = absint(WS_Form_Common::get_query_var_nonce('offset', '', $parameters));
			$remaining = false;
			$progress = 0;
			$processed = 0;
			$created = 0;
			...
			foreach($wsf_field_mapping['source'] as $index => $field_id_source) { 
				...
				$field_id_destination = absint($wsf_field_mapping['destination'][$index]);
		...
		public function api_field_mapping() {
			...
			foreach($wsf_field_mapping['source'] as $field_mapping_index => $field_id_source) {
				...
				$field_id_source = absint($field_id_source);
				$field_id_destination = absint($wsf_field_mapping['destination'][$field_mapping_index]);
