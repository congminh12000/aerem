<?php
	...
	class WS_Form_API {
		...
		public function api_json_response($data = false, $form_id = 0, $history = false, $form_full = true, $form_published = false, $form_parse = false, $form_public = false) {
			...
			if($form_id > 0) {
				...
				if(is_array($history)) {
					...
					$history['date'] = WS_Form_Common::wp_version_at_least('5.3') ? current_datetime()->format(get_option('date_format')) : gmdate(get_option('date_format'), current_time('timestamp'));
					$history['time'] = WS_Form_Common::wp_version_at_least('5.3') ? current_datetime()->format(get_option('time_format')) : gmdate(get_option('time_format'), current_time('timestamp'));
		...
		public function api_rest_api_init() {
			...
			register_rest_route(WS_FORM_RESTFUL_NAMESPACE, '/form/stat/get-chart-data/', array('methods' => 'GET', 'callback' => array($plugin_api_form_stat, 'api_get_chart_data'), 'permission_callback' => function () { return WS_Form_Common::can_user('manage_options_wsform'); }));
		...
		public function api_access_denied() {
			...
			WS_Form_Common::echo_wp_json_encode(array('edition' => WS_FORM_EDITION, 'error' => true, 'error_message' => __('Access denied', 'ws-form')));
			exit;
		}
