	...
	class WS_Form_Public {
					...
					foreach($fields as $field_key => $field) {
						...
						switch($field_type) {
							...
							case 'password' :
								...
								$password_strength_meter = (WS_Form_Common::get_object_meta_value($field, 'password_strength_meter', '') == 'on');
								if($password_strength_meter) {
		...
		public function enqueue_external() {
			...
			if(!$this->enqueued_js_dropzonejs && apply_filters('wsf_enqueue_js_dropzonejs', $this->enqueue_js_dropzonejs)) {
				...
				wp_enqueue_script($this->plugin_name . '-external-dropzonejs', $external['dropzonejs_js'], $dependencies, '5.9.3', $jquery_footer);
