...
class CSS_Engine {
	...
	public function init() {
		...
		if ( Gutenberg::is_active() ) {
			...
			$gutenberg_enabled = $this->database_settings->get_var( 'option-gutenberg-enable' ) === 'on' ? true : false;
	...
	public function update_framework_css_files() {
		...
		if ( empty( $values ) ) {
			...
			$values = ( new Variables() )->load_defaults();
	...
	public function handle_database_upgrade( $current_version, $previous_version ) {
		 ...
		 * @since 2.7.0
		 */
		$this->database_settings->save_vars( $new_values ); // will trigger generate_all_css_files.
		Logger::log( sprintf( '%s: done', __METHOD__ ) );
	}
