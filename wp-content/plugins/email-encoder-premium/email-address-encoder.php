 ...
 * Requires PHP 7.2 or greater.
 *
 * @param array $data Array to walk and encode
 *
...
function eae_encode_json_recursive( $data ) {
    ...
    array_walk_recursive( $data, function ( &$item, $key ) use ( $regexp, $callback, $atSignCheck ) {
        ...
        if ( in_array( strtolower( $key ), [ 'email', 'telephone' ], true ) ) {
            $item = $callback( [ $item ] );
 ...
 */
function eae_buffer() {
    if ( get_option( 'eae_search_in', 'filters' ) !== 'output' ) {
        return;
    }

    if ( apply_filters( 'eae_pre_buffer', false ) ) {
        return;
    }
...
function eae_buffer_callback( $buffer ) {
    if ( apply_filters( 'pre_parse_buffer', false ) ) {
        return $buffer;
    }
...
function eaep_activation_hook() {
    ...
    if ( get_option( 'eae_search_in', 'not-found' ) === 'not-found' ) {
        update_option( 'eae_search_in', 'filters' );
    }

    if ( get_option( 'eae_technique', 'not-found' ) === 'not-found' ) {
        update_option( 'eae_technique', 'entities' );
    }

    if ( get_option( 'eae_filter_priority', 'not-found' ) === 'not-found' ) {
