            ...
            <div class="manage-plans">
                        ...
                        <div class="manage-plan">
                            <div class="renewal-plan-name">
                                ...
                                <?php if($licenseData['license'] == "expired") { ?>
                                    <span class="expired-label"><?php esc_html_e("Expired", "folders") ?></span>
                                <?php } ?>
                            </div>
                    ...
                    <?php if($licenseData['license'] == "expired") { ?>
                        <div class="manage-plan">
                            <div class="plan-website-title">
                                <?php esc_html_e("Website Limit", "folders"); ?>
                            </div>
                            <div class="plan-website-limit">
                                (<?php echo sprintf(esc_html__("%d of %d websites used", "folders"), $siteCount, $totalActivation); ?>)
                            </div>
                        ...
                        <div class="manage-plan manage-plan-button">
                            <a class="table-upgrade-button" href="<?php echo esc_url($currentURL) ?>" target="_blank"><?php esc_html_e("Renew Plan", "folders") ?></a>
                        </div>
                    <?php } else {?>
                        <div class="manage-plan">
                            <div class="plan-website-title">
                        ...
                        <div class="manage-plan">
                            <div class="renewal-title"><?php esc_html_e("Renewal Date", "folders"); ?></div>
                            <div class="renewal-date">
                                <?php if($expireDate == "lifetime") {
                                    esc_html_e("You have lifetime version", "folders");
                                } else {
                                    ...
                                    if(isset($response['data']['rec_amount'])) { ?>
                                        <div class="renewal-desc">
                                            <?php echo sprintf(esc_html__("You will be billed %s", "folders"), "<b>$".$response['data']['rec_amount']."</b>") ?>
                                        </div>
                                    <?php }
                                } ?>
                            </div>
                            <?php if($siteCount >= $totalActivation) { ?>
                                <a class="table-upgrade-button" href="https://go.premio.io/upgrades/" target="_blank"><?php esc_html_e("Upgrade Plan", "folders") ?></a>
                            <?php } ?>
                        </div>
                    <?php } ?>
                </div>
                <?php if($licenseData['license'] == "expired") { ?>
                    <div class="manage-plan-footer">
                        ...
                        <?php esc_html_e("Your license has expired. Please renew your plan to keep getting feature updates and support.", "folders") ?>
                    </div>
                <?php } ?>
            </div>
                            ...
                            <div class="plan-col plan-detail plan-info" data-plan="1_website">
                                ...
                                <span class="plan-button">
                                    <?php if($planPriceId == $plans['1_website']['1_year']['price_id']) { ?>
                                        <a class="purchase-link current-plan" target="<?php echo esc_attr($currentTarget) ?>" href="<?php echo $currentURL ?>"><?php echo esc_attr($currentText) ?></a>
...
<div class="hide-plan-page">
    ...
    <a href="#" id="hide-plan-page"><?php esc_html__('Hide "Manage Your Plan" page', "chaty") ?></a>
</div>
...
<script type="text/javascript">
    (function (factory) {
        ...
        var currentPlan = parseInt(<?php echo esc_attr($response['data']['price_id']) ?>);
        // var currentPlan = 41;
        $(document).ready(function(){
            ...
            $(document).on("click", "#hide-plan-page", function(e){
                ...
                if(confirm("<?php esc_html_e("Are you sure?", "chaty"); ?>\nTo view this page again, you'll need to deactivate and reactivate the license key.")) {
                    window.location = "<?php echo admin_url("admin.php?page=chaty-app-upgrade&action=hide_chaty_plans&token=".wp_create_nonce("hide_chaty_plans")); ?>"
                }
            });
                ...
                $(document).on("change", "select.multiple-options", function(){
                    ...
                    if(typeof pluginPlans[planName] !== "undefined" && typeof pluginPlans[planName][selectedId] !== "undefined") {
                        ...
                        if(currentPlan == parseInt(selectedPlan.price_id)) {
                            $(this).closest(".plan-info").find(".purchase-link").attr("href", "<?php echo $currentURL ?>");
