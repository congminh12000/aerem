        ...
        <div class="easy-modal-inner flex items-center shadow-lg shadow-cht-gray-150/10 rounded-lg bg-white py-14 md:py-0">
                <div class="px-5 xl:px-20 flex-1">
                    ...
                    if (isset($m) && !empty($m)) {
                        ...
                        case "invalid":
                            $class_name = "error";
                            $message    = "<span class='dashicons dashicons-no-alt bg-[#FF424D] rounded-full text-white'></span> <span>".sprintf(esc_html__("Your license is invalid. Please get a %s to activate the product", "chaty"), "<a href='https://premio.io/downloads/chaty/' class='underline' target='_blank'>".esc_html__("valid license", "chaty")."</a>")."</span>";
                            break;
                        case "no_activations":
                            $class_name = "error";
                            $message    = "<span class='dashicons dashicons-no-alt bg-[#FF424D] rounded-full text-white'></span> <span>".sprintf(esc_html__("Your license was activated for another domain, please visit your %s", "chaty"), "<a href='https://premio.io/downloads/chaty/' class='underline' target='_blank'>".esc_html__("Premio account", "chaty")."</a>")."</span>";
                            break;
                        }//end switch
                        ?>
                            ...
                            <div>
                                ...
                                <div class="font-primary text-base text-cht-gray-150 pb-6 mx-auto">
                                    ...
                                    if (!$isLicenseActive) {
                                        ...
                                        echo sprintf(esc_html__("Your license will expire on %s", "chaty"), date("d M, Y", strtotime($licenseData['expires'])));
                                    } else if ($isLicenseActive == 2) {
                                        ?> <span class='error-message'> <?php
                                            echo sprintf(esc_html__("Your license has been expired on %s", "chaty"), date("d M, Y", strtotime($licenseData['expires'])));
                                        ?> </span> <?php
                                    }
                                    ?>
                                </div>

                                <div class='mb-5 items-start space-x-2 testimonial-<?php esc_html_e($class_name) ?>-message'>
                                    <?php echo $message; ?>
                                </div>
                            </div>
    ...
    function submitChatyLicenceForm($element) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                window.location = "<?php echo admin_url("admin.php?page=chaty-app-upgrade&m=") ?>" +
                    response;
            }
        })
    }
