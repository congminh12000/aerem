...
class Chaty_SL_Plugin_Updater
{
    ...
    public function check_update($_transient_data)
    {
        ...
        if (false === $version_info || null === $version_info) {
    ...
    public function show_update_notification($file, $plugin)
    {
        ...
        if (! empty($update_cache->response[$this->name]) && version_compare($this->version, $version_info->new_version, '<')) {
            ...
            ?>
            <tr class="plugin-update-tr" id="<?php esc_attr_e($this->slug) ?>'-update" data-slug="<?php esc_attr_e($this->slug) ?>" data-plugin="<?php esc_attr_e($this->slug) ?>/<?php esc_attr_e($file) ?>">
    ...
    private function api_request($_action, $_data)
    {
        ...
        if (! is_array($edd_plugin_url_available) || ! isset($edd_plugin_url_available[$store_hash])) {
            ...
            if (empty($host)) {
                ...
                $edd_plugin_url_available[$store_hash] = true;
            }
        }
    ...
    public function show_changelog()
    {
        ...
        $edd_sl_action = isset($request['edd_sl_action']) ? $request['edd_sl_action'] : "";
        ...
        $cache_key    = md5('edd_plugin_'.sanitize_key($plugin).'_'.$beta.'_version_info');
        ...
        if (! empty($version_info) && isset($version_info->sections['changelog'])) {
            ?><div style="background:#fff;padding:10px;"><?php esc_html_e($version_info->sections['changelog']) ?></div><?php
        }
