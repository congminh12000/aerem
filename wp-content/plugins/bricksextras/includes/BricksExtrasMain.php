...
class BricksExtrasMain {
	...
	public function __construct( $prefix, $version ) {
		...
		add_action( 'wp_ajax_nopriv_check_poster', array( $this, 'check_poster') );
		add_action( 'wp_ajax_check_poster', array( $this, 'check_poster') );
		...
		add_action( 'wp_enqueue_scripts', array( $this, 'framework_converter' ) );
	...
	function set_files() {
		$this->elements = [
			...
			'event_calendar' => [
				'title' => 'Calendar',
				'file_name' => 'x-event-calendar',
				'docs_slug' => 'event-calendar'
			],
			*/
			'fluent_form' => [
				'title' => 'Fluent Form',
	...
	function check_poster() {
		...
		$mediaPlayerDir = $thumbnail_dir_info['basedir'].'/xmediaplayer';
		$mediaPlayerUrl = $thumbnail_dir_info['baseurl'].'/xmediaplayer';

		if ( !file_exists($mediaPlayerDir) ) {
			mkdir($mediaPlayerDir);
		}

		$externalPosterSrc = isset ( $_POST['poster_src'] ) ? sanitize_text_field( $_POST['poster_src'] ) : false;
		$videoID = isset( $_POST['video_src'] ) ? \BricksExtras\Helpers::get_video_id( sanitize_text_field( $_POST['video_src'] ) ) : false;

		if (!$externalPosterSrc || !$videoID ) { return; }
		
		$newVideoDir = $thumbnail_dir_info['basedir'] . '/xmediaplayer/' . $videoID;
		$newVideoURL = $thumbnail_dir_info['baseurl'] . '/xmediaplayer/' . $videoID;
		...
		file_put_contents($newVideoDir . '/' . basename($path), file_get_contents( $externalPosterSrc ) );

		$currentPosters = [];

		if ( !get_option( 'bricksextras_media_poster_url') ) {

			$currentPosters[$videoID] = $newVideoURL . '/' . basename($path);
			add_option( 'bricksextras_media_poster_url', json_encode( $updatedPosters ));

		} else {
			$currentPosters = json_decode( get_option( 'bricksextras_media_poster_url'), TRUE);
			$currentPosters[$videoID] = $newVideoURL . '/' . basename($path);
			$updatedPosters = json_encode($currentPosters);
	...
	function framework_converter() {

		$convertCSSVariables = false;

		/* allow for disabling */
		$convertCSSVariables = apply_filters( 'bricksextras/convertCSSVariables', $convertCSSVariables );

		if ( $convertCSSVariables && class_exists( '\Automatic_CSS\Framework\Core' ) ) {
			wp_enqueue_style( 'x-variables', BRICKSEXTRAS_URL . 'components/assets/css/variables.css',[], $this->version );
		}
