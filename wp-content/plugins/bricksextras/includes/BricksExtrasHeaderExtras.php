...
class HeaderExtras {
	...
	public function extras_header() {
		...
		$xHeaderZindex = !!Helpers::getCurrentTemplateSetting('xHeaderZindex') ? Helpers::getCurrentTemplateSetting('xHeaderZindex') : '';
		$xStickyHeaderZindex = !!Helpers::getCurrentTemplateSetting('xStickyHeaderZindex') ? Helpers::getCurrentTemplateSetting('xStickyHeaderZindex') : '999';
	...
	public function extras_header_attributes( $attributes ) {
		...
		if ( 'true' === Helpers::getCurrentTemplateSetting('xStickyHeaderScroll') ) {
			...
			$attributes['data-x-scroll'] = Helpers::getCurrentTemplateSetting('xStickyHeaderScrollDistance') ? Helpers::getCurrentTemplateSetting('xStickyHeaderScrollDistance') : 0;
			...
			$attributes['data-x-hide-effect'] = Helpers::getCurrentTemplateSetting('xStickyHeaderHideEffect') ? Helpers::getCurrentTemplateSetting('xStickyHeaderHideEffect') : 'slideUp';

			/* hide header */

			if ( 'true' === Helpers::getCurrentTemplateSetting('xStickyHeaderHide') ) {

				$attributes['data-x-header-hide'] = Helpers::getCurrentTemplateSetting('xStickyHeaderHideDistance') ? Helpers::getCurrentTemplateSetting('xStickyHeaderHideDistance') : 0;
				$attributes['data-x-header-tollerance'] = Helpers::getCurrentTemplateSetting('xStickyHeaderHideTollerance') ? Helpers::getCurrentTemplateSetting('xStickyHeaderHideTollerance') : 0;
				wp_enqueue_script( 'x-hide-header', BRICKSEXTRAS_URL . 'components/assets/js/hideheader.js', '', '1.0.0', true );

			}

			wp_enqueue_script( 'x-sticky-header', BRICKSEXTRAS_URL . 'components/assets/js/header.js', '', '1.0.1', true );
		 ...
		 $attributes['data-x-overlay'] = $overlayHeaderAttr;
