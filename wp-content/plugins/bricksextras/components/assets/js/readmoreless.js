  ...
  _babel_runtime_helpers_createClass__WEBPACK_IMPORTED_MODULE_1___default()(Readmore, [{
    ...
    value: function toggle() {
      ...
      var toggleElement = function toggleElement(element) {
        ...
        var expanded = parseInt(element.getBoundingClientRect().height) <= parseInt(element.readmore.collapsedHeight);
...
function xReadMoreLess(){

  if ( document.querySelector('body > .brx-body.iframe') ) {
      return
  }

    const extrasReadmore = function ( container ) {

      container.querySelectorAll(".brxe-xreadmoreless").forEach((readMore) => {

        const configAttr = readMore.getAttribute('data-x-readmore'),
		          config = configAttr ? JSON.parse(configAttr) : {};

        const readMoreContent = readMore.querySelector('.x-read-more_content')
              openText = config.moreText ? config.moreText : '',
              closeText = config.lessText ? config.lessText : '',
              moreAria = config.moreAria ? config.moreAria : '',
        ...
        if ( null != config.icon ) {
          ...
          if ( 'svg' === config.icon.library ) {
            let svgIcon = readMore.nextSibling.innerHTML;
            buttonIcon = '<span class="x-read-more_link-icon">' + svgIcon + '</span>';
          } else {
            buttonIcon = '<span class="x-read-more_link-icon"><i class="' + config.icon.icon + '"></i></span>';
          }
        ...
        function doReadMore() {
          ...
          new Readmore(readMoreContent, {

                  speed: config.speed ? parseInt(config.speed) : 300,
                  moreLink: moreText,
                  lessLink: lessText,
                  embedCSS: false,
                  ...
                  beforeToggle: function(trigger, element, expanded) {
                    ...
                    if(!expanded) {
                      ...
                      readMoreContent.classList.remove('x-read-more_expand');
                      readMore.dispatchEvent(new Event('x_readmore:collapse'))
                    }
                  },
                  afterToggle: function(trigger, element, expanded) {
                    if(expanded) {
                      readMore.dispatchEvent(new Event('x_readmore:expanded'))
                    } else {
                      readMore.dispatchEvent(new Event('x_readmore:collapsed'))
                    }
                  },
    ...
    function xReadMoreJAX(e) {
      ...
      if (typeof e.detail.queryId === 'undefined') {
        if ( typeof e.detail.popupElement === 'undefined' ) {
          ...
          extrasReadmore( e.detail.popupElement );
        }
      } else {
        ...
        setTimeout(() => {
          ...
          if ( !queryLoopEl || !queryLoopEl.querySelector('.brxe-xreadmoreless') ) {
            return;
          } else {
              extrasReadmore( document.querySelector('.brxe-' + e.detail.queryId).parentElement ) 
          }
        }, 0);
