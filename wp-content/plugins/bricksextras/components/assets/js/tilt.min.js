...
function xTilt() {
    ...
    const extrasTilt = function ( container ) {

        let tiltStatus = false;
        let mobileStatus = false;

        const xTiltElements = container.querySelectorAll("[data-x-tilt]");
    ...
    function xTiltAJAX(e) {

        if (typeof e.detail.queryId === 'undefined') {
            if ( typeof e.detail.popupElement === 'undefined' ) {
                return;
            } else {
                extrasTilt( e.detail.popupElement )
            }
        }

        setTimeout(() => {
            if ( document.querySelector('.brxe-' + e.detail.queryId) ) {
                    extrasTilt(document.querySelector('.brxe-' + e.detail.queryId).parentElement);
            }
        }, 0);
      }
      
      document.addEventListener("bricks/ajax/load_page/completed", xTiltAJAX)
      document.addEventListener("bricks/ajax/pagination/completed", xTiltAJAX)
      document.addEventListener("bricks/ajax/popup/loaded", xTiltAJAX)
      document.addEventListener("bricks/ajax/end", xTiltAJAX)

    // Expose function
    window.doExtrasTilt = extrasTilt;

}

document.addEventListener("DOMContentLoaded",function(e){
    bricksIsFrontend&&xTilt()
});