...
function xLightbox(){
    ...
    let xLightboxFocus = {}; 
      ...
      const extrasLightbox = function ( container, ajax ) {
        ...
        container.querySelectorAll('.brxe-xdynamiclightbox').forEach((dynamicLightbox, index) => {
            ...
            const configAttr = dynamicLightbox.getAttribute('data-x-lightbox')
            const config = configAttr ? JSON.parse(configAttr) : {}

            const lightboxContent = dynamicLightbox.querySelector('.x-dynamic-lightbox_content')

            if ( lightboxContent ) {

                if ( lightboxContent.querySelector('.brxe-xreadmoreless') ) {
                    lightboxContent.querySelector('.brxe-xreadmoreless').style.opacity = 0
                }

                if ( lightboxContent.querySelector('[loading="lazy"]') ) {
                    lightboxContent.querySelectorAll('[loading="lazy"]').forEach((lazyAsset) => {
                        lazyAsset.setAttribute('loading','eager')
                    })
                }

            }

            const lightboxClassesArray = [].slice.apply(dynamicLightbox.classList)
            ...
            const navButtons = 'true' === config.grouping ? `<button class="gnext gbtn" tabindex="0" aria-label="Next" data-customattribute="example">{nextSVG}</button>
            ...
            if ( 'false' !== config.allowID ) {
                ...
                if ( null != config.isLooping ) {
                    ...
                    wrapperID = dynamicLightbox.id;
                } 

                lightboxHTML = `
                    <div id=glightbox-body class="glightbox-container ` + config.captionPosition + ` glightbox-container-id ` + wrapperClass + ` " data-lenis-prevent>
            ...
            if ( 'true' === config.grouping ) {
                lightboxIdentifier = config.linkSelector;
            } else {
                lightboxIdentifier = '[data-x-lightbox-link="' + lightboxLinkIdentifier + '"]';
            }

            /* inline content as source */

            if ('inline' === config.contentType) {

                if ( insideLoop ) {

                    dynamicLightbox.closest('.brxe-' + config.isLooping).querySelectorAll(config.linkSelector).forEach((linkSelector) => {
                        ...
                        linkSelector.setAttribute('href','#x-dynamic-lightbox_content-' + config.identifier + '-' + count)

                     })

                } else {

                    if ( document.querySelectorAll(config.linkSelector) ) {
            ...
            if ( 'manual' === config.contentType ) {

                if ( document.querySelector(config.linkSelector) ) {
                    ...
                    console.log('BricksExtras: No links found, check the selector is correct')
                }

            }

            if ('inline' === config.contentType) {
                clickSelector = lightboxIdentifier
            } else if ('manual' === config.contentType) {
                clickSelector = '[data-x-lightbox-link="' + lightboxLinkIdentifier + '"]'
            } else {
                clickSelector = '.x-dynamic-lightbox_link-' + lightboxLinkIdentifier
            }

            let touchNavigation = true,
                keyboardNavigation = config.keyboardNavigation,
                draggable = config.rawConfig.draggable,
                returnTrigger = '',
                newIndex,
                animation = 'fade' === config.reduceMotion ? 'fade' : 'x_animation';


            /* make sure no sliding going on if no grouping */
            if ( 'true' !== config.grouping || 'calendar' === config.contentType ) {
            ...
            if ( document.querySelector(clickSelector) ) {
                ...
                setTimeout(() => {
                    ...
                    setTimeout(function () {
                        ...
                        dynamicLightbox.dispatchEvent(new Event('x_lightbox:init'))
                    }, 300)

                    function closeClick(e) {
                        if ( e.target.hasAttribute('data-x-lightbox-close') ) {
                    ...
                    lightboxInstance.on('open', () => {
                        if ( document.querySelector('.glightbox-wrapper') ) {
                            document.querySelector('.glightbox-wrapper').classList.add(...lightboxClassesArray)
                        } else {
                            document.querySelector('.glightbox-container').classList.add(...lightboxClassesArray)
                        }
                        dynamicLightbox.dispatchEvent(new Event('x_lightbox:open'))
                        ...
                        /* hide nav if no grouping */
                        if ( 'true' !== config.grouping ) {
                            document.querySelectorAll('.glightbox-container .gbtn:not(.gclose)').forEach((navButton) => {
                    ...
                    lightboxInstance.on('close', () => {
                        dynamicLightbox.dispatchEvent(new Event('x_lightbox:close'))
                    ...
                    lightboxInstance.on('slide_after_load', (data) => {

                        dynamicLightbox.dispatchEvent(new Event('x_lightbox:after_slide_load'))
                        if( typeof bricksLazyLoad === "function" ){
                            bricksLazyLoad()
                        }
                        ...
                        xLightboxFocus.prevFocus = trigger;

                        /* iframe */
                        if ('iframe' === config.contentType || 'manual' === config.contentType) {

                            const iframeEl = slideNode.querySelector('iframe');
                            ...
                            if ( iframeEl && !!(iframeEl.contentDocument) ) {
                                ...
                                if ( 'false' != config.contentSelector ) {

                                    if ( iframeDoc.querySelector(config.contentSelector) ) {
                                        const visibleContent = iframeDoc.querySelector(config.contentSelector)
                                        iframeDoc.querySelector('body').prepend(visibleContent)
                                        iframeDoc.querySelectorAll('body > *:not(script):not(style):not(svg):not(' + config.contentSelector + ')').forEach((hiddenContent) => {
                                            hiddenContent.style.display = 'none'
                                        })
                                    }
                                }   

                                var clientHeight = iframeDoc.querySelector('body').clientHeight;

                                if ( null == config.height ) {
                                    slideNode.querySelector('.ginner-container').style.height = clientHeight + 'px'
                                }
                        ...
                        if (slideNode.querySelector('[data-x-lightbox-prev]')) {
                            slideNode.querySelectorAll('[data-x-lightbox-prev]').forEach(prevButton => {
                        ...
                        /* next slide when clicked */
                        if (slideNode.querySelector('[data-x-lightbox-next]')) {
                            slideNode.querySelectorAll('[data-x-lightbox-next]').forEach(nextButton => {
                        ...
                        if ( slideNode.querySelector('.brxe-xreadmoreless') ) {
                            ...
                            slideNode.querySelectorAll('.brxe-xreadmoreless').forEach(readMore => {
                        ...
                        /* slider */
                        if ( slideNode.querySelector('.x-slider') ) {
                            doExtrasSlider(slideNode)
                        }
                        ...
                        if ( 'false' !== config.hashToClose ) {
                            slideNode.querySelectorAll("a[href*='#']:not(.menu-item-has-children > a)").forEach((hashLink) => {
                        ...
                        /* resize to make sure any sliders etc function correctly */
                        if (typeof(Event) === 'function') {
                            setTimeout(() => {

                                /* workaround for incorrect splide pagination number */
                                if ( slideNode.querySelector('.splide__pagination') ) {

                                    window.dispatchEvent(new Event('resize'));
                                    slideNode.querySelector('.splide__pagination').style.display = 'none';

                                    setTimeout(() => {
                                        if ( slideNode.querySelector('.splide__pagination') ) {
                                            let activePaginations = slideNode.querySelectorAll('.splide__pagination .is-active')
                                            
                                            if ( 1 !== activePaginations.length ) {

                                                var last = activePaginations[activePaginations.length- 1]
                                                
                                                while (!!last.parentElement.previousElementSibling) {
                                                    last.parentElement.previousElementSibling.remove();
                                                }
                                                slideNode.querySelector('.splide__pagination').style.display = 'flex';
                                            }
                                        }
                                    }, 500)
                                }
                        ...
                        setTimeout(() => {
                            ...
                            if ( slideNode.classList.contains('current') ) {

                                let focusable = slideNode.querySelector('.x-dynamic-lightbox_content')
                    ...
                    lightboxInstance.on('slide_changed', ({ prev, current }) => {
                        ...
                        if ( slideNode ) {
                            ...
                            if ( 'false' !== config.returnFocus ) {

                                let allsiblings = siblings(current.slideNode)

                                allsiblings.forEach((sibling) => {

                                    if ( sibling.classList.contains('current') ) {
                    ...
                    lightboxInstance.on('close', () => {
                        if ( xLightboxFocus.prevFocus && 'false' !== config.returnFocus ) {
                           if ( !xLightboxFocus.prevFocus.closest('.splide__slide') ) {
                                xLightboxFocus.prevFocus.focus()
                            }
                        }
    ...
    function xLightboxAJAX(e) {
        ...
        if (typeof e.detail.queryId === 'undefined') {
            if ( typeof e.detail.popupElement === 'undefined' ) {
                ...
                extrasLightbox( e.detail.popupElement, true )
            }
        }
   ...
   document.addEventListener("bricks/ajax/popup/loaded", xLightboxAJAX)
   document.addEventListener("bricks/ajax/end", xLightboxAJAX)
