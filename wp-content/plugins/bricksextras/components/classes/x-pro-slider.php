...
class X_Pro_Slider extends \Bricks\Element {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'x-slider', BRICKSEXTRAS_URL . 'components/assets/js/proslider.js', '', '1.1.4', true );
		...
		if (! \BricksExtras\Helpers::elementCSSAdded($this->name) ) {
			wp_enqueue_style( 'x-slider', BRICKSEXTRAS_URL . 'components/assets/css/proslider.css', [], '' );
		}
	...
	public function set_controls() {
		...
		*/
		...
		$this->controls['releaseWheel'] = [
			...
			'inline'   => true,
			'tooltip'  => [
				'content'  => 'Release the wheel event when reaches the first or last slide',
				'position' => 'top-left',
			],
		  ]; 
		...
		$this->controls['conditionalSep'] = [
			'group'    => 'conditionalSlider',
			'label' => esc_html__( 'Horizontal layout if slider not active', 'bricks' ),
			'type'     => 'separator',
	...
	public function render_arrows() {
		...
		$arrows = isset( $this->settings['arrows'] ) ? $this->settings['arrows'] : 'true';

		$nextAria = !empty( $settings['i18nNext'] ) ? esc_attr__( $settings['i18nNext'] ) : esc_attr__('Next slide');
		$prevAria = !empty( $settings['i18nPrev'] ) ? esc_attr__( $settings['i18nPrev'] ) : esc_attr__('Previous slide');

		if ( ( ! $prev_arrow && ! $next_arrow ) || 'false' === $arrows ) {
			return;
		}
	...
	public function render() {
		...
		$rootMargin = ! empty( $settings['rootMargin'] ) ? $settings['rootMargin'] : '0px';
		...
		$listTag = isset( $settings['listTag'] ) ? esc_html($settings['listTag']) : 'div';
		...
		if ( isset( $settings['wheel'] ) ) {
			...
			$splide_options['rawConfig']['wheelSleep'] = ! empty( $settings['wheelSleep'] ) ? intval( $settings['wheelSleep'] ) : 300;
		...
		if ( 'autoscroll' === $autoplayscroll ) {
			wp_enqueue_script( 'x-splide-autoscroll', BRICKSEXTRAS_URL . 'components/assets/js/splide-autoscroll.js', '', '0.5.3', true );
		}
