...
class X_Dynamic_Lightbox extends \Bricks\Element {
  ...
  public function enqueue_scripts() {
	...
	wp_enqueue_script( 'x-dynamic-lightbox', BRICKSEXTRAS_URL . 'components/assets/js/dynamiclightbox.js', '', '1.2.0', true );
	...
	if (! \BricksExtras\Helpers::elementCSSAdded($this->name) ) {
		wp_enqueue_style( 'x-dynamic-lightbox', BRICKSEXTRAS_URL . 'components/assets/css/dynamiclightbox.css', [], '' );
	}
  ...
  public function render() {
	...
	$allowID = apply_filters( 'bricksextras/dynamiclightbox/allow_id_styling', $allowID );
	 ...
	 $lightbox_config = [
		...
		'linkSelector' => isset( $this->settings['linkSelector'] ) ? $this->settings['linkSelector'] : 'false',
		'sliderSelector' => isset( $this->settings['sliderSelector'] ) ? $this->settings['sliderSelector'] : 'false',
		'contentSource' => isset( $this->settings['contentSource'] ) ? $this->settings['contentSource'] : 'false',
		'contentSelector' => isset( $this->settings['contentSelector'] ) ? $this->settings['contentSelector'] : 'false',
	...
	if ( isset( $this->settings['lightboxContentHeight'] ) ) {
		$lightbox_config += [ 'height' => $this->settings['lightboxContentHeight']];
	} 

	if ( isset( $this->settings['lightboxContentWidth'] ) ) {
		$lightbox_config += [ 'width' => $this->settings['lightboxContentWidth']];
	}
	...
	if ( method_exists('\Bricks\Query','is_any_looping') ) {
		...
		if ( $query_id ) {
			$lightbox_config += [ 'isLooping' => \Bricks\Query::get_query_element_id( $query_id ) ];
			...
			$lightboxContentIdentifier = "x-dynamic-lightbox_content-" . $this->id . "-" . $loopIndex;

			$count = $loopIndex;
			
		} else {
			$this->set_attribute( '_root', 'data-x-id', $this->id );
			$lightboxContentIdentifier = "x-dynamic-lightbox_content-" . $this->id . "-0";
		}
	...
	if ($maybeCaptions) {
	 ...
	 if ( 'inline' === $lightboxContent ) {
		...
		$contentSource = isset( $this->settings['contentSource'] ) ? $this->settings['contentSource'] : '/';
	 }
	 ...
	 if ( 'iframe' === $lightboxContent ) {
		...
		echo "<a href=" . $contentSource . " class='x-dynamic-lightbox_link x-dynamic-lightbox_link-" . $this->id . " x-dynamic-lightbox_link-" . $this->id . '-' . $count . "' ";

		$lightboxContentWidth = isset( $this->settings['lightboxContentWidth'] ) ? $this->settings['lightboxContentWidth'] : '600px';

		echo "data-width='" . $lightboxContentWidth . "' ";
		echo isset( $this->settings['lightboxContentHeight'] ) ? "data-height='" . $this->settings['lightboxContentHeight'] . "' " : "";
	...
	if ( 'calendar' === $lightboxContent ) {
		...
		echo "<a data-x-template-id='" . $template_id . "' href='#" . $lightboxContentIdentifier . "' class='x-dynamic-lightbox_link x-dynamic-lightbox_link-" . $this->id . " x-dynamic-lightbox_link-" . $this->id . '-' . $count . "' ";
		...
		echo "<div class='x-dynamic-lightbox_calendar-content' id='" . $lightboxContentIdentifier  . "'>";
	...
	if ( 'gallery' === $lightboxContent ) {
		...
		echo "<button tabindex=0 class='x-dynamic-lightbox_link x-dynamic-lightbox_link-" . $this->id . " x-dynamic-lightbox_link-" . $this->id . '-' . $count . "' ";
	...
	if ('inline' === $lightboxContent ) {
		...
		echo "<div class='x-dynamic-lightbox_content' tabindex='0' id='" . $lightboxContentIdentifier  . "' >";
