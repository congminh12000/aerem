...
class X_Burger_Trigger extends \Bricks\Element {
  ...
  // Element properties
	...
	public function enqueue_scripts() {
		wp_enqueue_script( 'x-burger', BRICKSEXTRAS_URL . 'components/assets/js/burgertrigger.js', '', '1.0.1', true );
		if (! \BricksExtras\Helpers::elementCSSAdded($this->name) ) {
			wp_enqueue_style( 'x-burger', BRICKSEXTRAS_URL . 'components/assets/css/burgertrigger.css', [], '' );
		  }
	}

	public function render() {

		$animation_class = isset( $this->settings['burger_animation'] ) ? $this->settings['burger_animation'] : '';
