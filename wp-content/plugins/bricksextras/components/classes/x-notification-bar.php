...
class X_Notification_Bar extends \Bricks\Element {
  ...
  public function enqueue_scripts() {
    ...
    wp_enqueue_script( 'x-frontend', BRICKSEXTRAS_URL . 'components/assets/js/frontend.js', '', '1.0.0', true );
		wp_enqueue_script( 'x-notification-bar', BRICKSEXTRAS_URL . 'components/assets/js/notificationbar.js', ['x-frontend'], '1.0.2', true );

    if (! \BricksExtras\Helpers::elementCSSAdded($this->name) ) {
			wp_enqueue_style( 'x-notification-bar', BRICKSEXTRAS_URL . 'components/assets/css/notificationbar.css', [], '' );
		}
  ...
  public function render() {
    ...
    $stickyDisplay = isset( $this->settings['stickyDisplay'] ) ? $this->settings['stickyDisplay'] : "always" ;
    ...
    if ( 'wysiwyg' === $notification_content ) {
     if ( isset( $this->settings['notification_wysiwyg'] ) ) {
       echo wp_kses_post( $this->settings['notification_wysiwyg'] );
     }
   } else {
