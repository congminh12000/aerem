...
class X_Pro_Accordion extends \Bricks\Element {
	...
	public function get_nestable_item() {
		...
		$alternativeStructure = apply_filters( 'bricksextras/proaccordion/usebuttondefault', $alternativeStructure );
		...
		if ( $alternativeStructure ) {
			...
			return [
				...
				'children' => [
					[
						'name'     => 'block',
						'label'    => esc_html__( 'Heading wrapper', 'bricks' ),
						'settings' => [
							'tag' => 'h4',
							'_hidden'         => [
		...
		if ( $query_id ) {
			...
			if ( BricksExtras\Helpers::get_bricks_looping_parent_query_id_by_level(2) ) {
				...
				if ( BricksExtras\Helpers::get_bricks_looping_parent_query_id_by_level(1) ) {
					...
					$loopIndexHash = \Bricks\Query::get_loop_index() + 1;
				}
			}			
	...
	echo $output;
		...
		if ( isset( $settings['faqSchema'] ) ) {
			...
			wp_enqueue_script( 'x-faq-schema', BRICKSEXTRAS_URL . 'components/assets/js/faq-schema.js', '', '1.0.0', true );
  ...
  public function enqueue_scripts() {
    ...
    wp_enqueue_script( 'x-accordion', BRICKSEXTRAS_URL . 'components/assets/js/proaccordion.js', ['x-frontend'], '1.0.8', true );

	if (! \BricksExtras\Helpers::elementCSSAdded($this->name) ) {
		wp_enqueue_style( 'x-accordion', BRICKSEXTRAS_URL . 'components/assets/css/proaccordion.css', [], '' );
	}
