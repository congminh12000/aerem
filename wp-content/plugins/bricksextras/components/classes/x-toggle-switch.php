...
class X_Toggle_Switch extends \Bricks\Element {
  ...
  public function enqueue_scripts() {
    wp_enqueue_script( 'x-toggle-switch', BRICKSEXTRAS_URL . 'components/assets/js/toggleswitch.js', '', '1.0.4', true );
    if (! \BricksExtras\Helpers::elementCSSAdded($this->name) ) {
      wp_enqueue_style( 'x-toggle-switch', BRICKSEXTRAS_URL . 'components/assets/css/toggleswitch.css', [], '' );
    }
  }
  ...
  public function render() {
    ...
    $leftLabel = isset( $this->settings['leftLabel'] ) ? esc_attr__( $this->settings['leftLabel'] ) : esc_attr__( 'Monthly' );
    $rightLabel = isset( $this->settings['rightLabel'] ) ? esc_attr__( $this->settings['rightLabel'] ) : esc_attr__( 'Yearly' );
    ...
    $whichSwitcher = isset( $this->settings['whichSwitcher'] ) ? $this->settings['whichSwitcher'] : 'section';
    $switcherSelector = isset( $this->settings['switcherSelector'] ) ? $this->settings['switcherSelector'] : '.my-content-switcher';

    
    $config = [
      'type' =>  $type,
      'contentSwitcher' => 'section' === $whichSwitcher ? 'section' : $switcherSelector
    ];
    ...
    if ('double' === $type ) {
      ...
      if (!$disableLabels) {
        echo '<span class="x-toggle-switch_label">' . esc_attr__( $leftLabel ) . '</span>';
      }

      echo 
           '<label class="x-toggle-switch_switch">
              <div class="screen-reader-text">' . esc_attr__( $screenReaderText ) . '</div>
      ...
      if (!$disableLabels) {
        echo '<span class="x-toggle-switch_label">' . esc_attr__(  $rightLabel ) . '</span>';
      }
      ...
      // Query Loop
        ...
        $query = new \Bricks\Query( [
				...
				$labelsQuery = $this->settings['labelsQuery'];
        ...
        $this->set_attribute( "x-toggle-switch_label-0", 'tabindex', '0' );
          ...
          foreach ( $this->settings['labels'] as $index => $label ) {
            ...
            if ( ! empty( $label['title'] ) ) {
              ...
              echo "<button {$this->render_attributes( "x-toggle-switch_label-$index" )}><span>" . esc_attr__( $label['title'] ) . "</span></button>";
  ...
  public function render_repeater_item( $labelsQuery ) {
    ...
    ob_start();
      ...
      if ( ! empty( $labelsQuery ) ) {
        ...
        echo "<button {$this->render_attributes( "x-toggle-switch_label-$index" )}><span>{$labelsQuery}</span></button>";
