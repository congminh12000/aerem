...
class X_Interactive_Cursor extends \Bricks\Element {
  ...
  public function set_controls() {
    ...
    $this->controls['builderPreview'] = [
        ...
        'type' => 'select',
        'info' => esc_html__( 'Preview each state in the builder to change the styles for each', 'bricks' ),
        'options' => [
  ...
  public function enqueue_scripts() {
    wp_enqueue_script( 'x-cursor', BRICKSEXTRAS_URL . 'components/assets/js/cursor.js', '', '1.0.5', true );

    if (! \BricksExtras\Helpers::elementCSSAdded($this->name) ) {
			wp_enqueue_style( 'x-cursor', BRICKSEXTRAS_URL . 'components/assets/css/interactivecursor.css', [], '' );
		}
  }
