...
class X_Table_Of_Contents extends \Bricks\Element {
  ...
  public function set_controls() {
	...
	$this->controls['collapseDepth'] = [
	  ...
	  $this->controls['conditionalDisplay'] = [
		'tab' => 'content',
		'label' => esc_html__( 'Remove if no headings found', 'bricks' ),
		'type' => 'select',
		'options' => [
  ...
  public function enqueue_scripts() {
	wp_enqueue_script( 'x-frontend', BRICKSEXTRAS_URL . 'components/assets/js/frontend.js', '', '1.0.1', true );
	wp_enqueue_script( 'x-toc', BRICKSEXTRAS_URL . 'components/assets/js/tableofcontents.js', ['x-frontend'], '1.0.5', true );

	if (! \BricksExtras\Helpers::elementCSSAdded($this->name) ) {
		wp_enqueue_style( 'x-toc', BRICKSEXTRAS_URL . 'components/assets/css/tableofcontents.css', [], '' );
	}
  }

  public function render() {

		$tag = isset( $this->settings['tag'] ) ? $this->settings['tag'] : 'nav';
		...
		$toc_config = [
			'contentSelector' => isset( $this->settings['contentSelector'] ) ? $this->settings['contentSelector'] : '',
			...
			'idPrefix' => isset( $this->settings['idPrefix'] ) ? $this->settings['idPrefix'] : 'heading',
			'autoIDText' => isset( $this->settings['autoIDText'] ) ? $this->settings['autoIDText'] : 'prefix',
			...
			'ignoreSelector' => isset( $this->settings['ignoreSelector'] ) ? $this->settings['ignoreSelector'] : '',
		...
		$this->set_attribute( 'x-toc_header', 'tabindex', '00' );
		$this->set_attribute( 'x-toc_header', 'aria-label', $aria_label );
		$this->set_attribute( 'x-toc_header', 'aria-controls', 'x-toc_' . $this->id );
		...
		$this->set_attribute( 'x-toc_body', 'id', 'x-toc_' . $this->id );
