...
class X_Dynamic_Table extends \Bricks\Element {
  ...
  public function set_controls() {
		...
		$this->controls['stackMinWidth'] = [
		  ...
		  $this->controls['stackPadding'] = [
			'tab' => 'content',
			'label' => esc_html__( 'Cell padding-left', 'bricks' ),
			'group'	=> 'mobile_group',
			'type' => 'number',
			'min' => 0,
			'max' => 1000,
  ...
  public function enqueue_scripts() {
	...
	wp_enqueue_script( 'x-table', BRICKSEXTRAS_URL . 'components/assets/js/dynamictable.js', '', '1.0.8', true );
	...
	if (! \BricksExtras\Helpers::elementCSSAdded($this->name) ) {
		wp_enqueue_style( 'x-table', BRICKSEXTRAS_URL . 'components/assets/css/dynamictable.css', [], '' );
	}
  ...
  public function render() {
		...
		if ( !isset( $settings['maybe_pagination'] ) || 'true' === $settings['maybe_pagination'] ) {
			$config += [
				'rowsPerPage' => isset( $settings['rows_per_page']  ) ? $settings['rows_per_page'] : 5,
		...
		$this->set_attribute( '_root', 'data-x-table', wp_json_encode( $config, JSON_UNESCAPED_UNICODE ) );
		...
		if ( 'dynamic' === $maybeDynamic ) {
			...
			if ( $content_items ) {			
				...
				foreach ( $content_items as $index => $content_item ) {
					...
					$header_title = ! empty( $content_item['title'] ) ? esc_attr__( $content_item['title'] ) : '';
					...
					echo '<th class="label" data-x-column="' . $content_widths . '" ' . $column_attribute . '>' . $header_title . '</th>';
		...
		if ( 'dynamic' === $maybeDynamic ) {
			...
			if ( $row_items ) {		
				...
				foreach ( $row_items as $index => $row_item ) {
					...
					echo $this->render_repeater_item_static( $row_item, $content_items_static );
	...
	<?php	
		...
		if ( $content_items ) {			
			...
			foreach ( $content_items as $index => $content_item ) {
				...
				$data_type = isset( $content_item['data_type'] ) ? 'number' : 'string';

				echo '<td data-x-type="'. $data_type .'">';

				$content_datas = ! empty( $content_item['data'] ) ? wp_kses( $content_item['data'], 'post' ) : '';
		...
		<?php	
				...
				if ( $content_items_static ) {			
					...
					foreach ( $content_items_static as $index => $content_item_static ) {
						...
						$data_type = isset( $content_item_static['data_type'] ) ? 'number' : 'string';
				...
				if ( isset( $row_item['cell'] ) ) {
					...
					foreach ( array_slice( $row_item['cell'], 0, sizeof($content_items_static) ) as $index => $cell ) {
						...
						echo '<td rowspan="2" data-x-type="' . $data_types[$index] . '">';
						if ($cellContent) {
							echo wp_kses( $cellContent , 'post');
						}
						echo '</td>';
